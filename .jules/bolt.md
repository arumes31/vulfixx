## 2026-07-11 - Preload active team name into session to avoid per-render DB query
...
## 2026-07-11 - Cache AES-GCM AEAD instances derived from environment variables
**Learning:** `cipher.AEAD` implementations derived from environment variables (like ENCRYPTION_KEY) do not maintain internal state between calls and are safe for concurrent use. Recomputing them on every encryption/decryption is expensive.
**Action:** Use a `sync.Map` to cache `cipher.AEAD` instances keyed by the environment variable value. This avoids recomputing the block and GCM instances while remaining safe for isolated tests that may alter the environment variable.
