## 2026-07-03 - Preload active team name into session to avoid per-render DB query
...
## 2026-07-03 - Add index to secondary column of UNIQUE constraint
**Learning:** PostgreSQL's `UNIQUE(user_id, cve_id)` automatically creates a composite index, which speeds up queries filtering by `user_id` (the first column) or both `user_id` and `cve_id`. However, it does not speed up queries filtering ONLY by `cve_id` (the secondary column). This results in a full table scan when querying by the secondary column.
**Action:** When creating tables with multi-column `UNIQUE` constraints where the secondary column is frequently queried alone (like `SELECT * FROM table WHERE secondary_id = $1`), explicitly create an independent index for that secondary column.
