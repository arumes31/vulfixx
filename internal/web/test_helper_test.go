...
func TestExpectBaseQueries(t *testing.T) {
	...
	t.Run("Valid User", func(t *testing.T) {
		...
		// Onboarding and sub count
		var completed bool
		var count int
		_ = mock.QueryRow(context.Background(), "SELECT u.onboarding_completed, (SELECT COUNT(*) FROM user_subscriptions WHERE user_id = $1) FROM users u WHERE u.id = $1", 123).Scan(&completed, &count)
