...
func TestExpectBaseQueries(t *testing.T) {
	...
	t.Run("Valid User", func(t *testing.T) {
		...
		// Team list
		_, _ = mock.Query(context.Background(), "SELECT t.id, t.name, (tm.user_id IS NOT NULL) AS is_member FROM teams t LEFT JOIN team_members tm ON t.id = tm.team_id AND tm.user_id = $1 WHERE tm.user_id = $1 OR t.id = $2", 123, 0)
