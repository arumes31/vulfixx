...
func (app *App) Routes(cfg *config.Config) (http.Handler, error) {
	...
	r.Method("GET", "/login", app.RateLimitMiddleware(app.LoginRateLimitMiddleware(http.HandlerFunc(app.LoginHandler))))
	r.Method("POST", "/login", app.RateLimitMiddleware(app.LoginRateLimitMiddleware(http.HandlerFunc(app.LoginHandler))))
