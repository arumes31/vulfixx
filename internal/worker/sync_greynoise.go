...
func (w *Worker) syncGreyNoise(ctx context.Context) {
	...
	var pendingUpdates []greynoiseUpdateItem
	...
	for i := 0; i < numWorkers; i++ {
		g.Go(func() error {
			for cveID := range cveChan {
				...
				hits, class, err := w.fetchGreyNoiseHits(gCtx, cveID)
				if err != nil {
				...
				resultsMu.Lock()
				pendingUpdates = append(pendingUpdates, greynoiseUpdateItem{
					cveID:          cveID,
					hits:           hits,
					classification: class,
				})
				if len(pendingUpdates) >= 50 {
					w.updateGreyNoiseBatch(gCtx, pendingUpdates)
					atomic.AddInt64(&processedCount, int64(len(pendingUpdates)))
					pendingUpdates = nil
				}
				resultsMu.Unlock()
			}
...
Loop:
	...
	if len(pendingUpdates) > 0 {
		w.updateGreyNoiseBatch(ctx, pendingUpdates)
		atomic.AddInt64(&processedCount, int64(len(pendingUpdates)))
	}
