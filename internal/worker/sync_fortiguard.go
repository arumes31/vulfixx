...
func (w *Worker) updateCVEsWithAdvisory(ctx context.Context, advisory *FortiGuardAdvisory) error {
	...
	for rows.Next() {
		...
		existingProducts := make(map[prodKey]bool, len(cve.AffectedProducts))
		for _, p := range cve.AffectedProducts {
			existingProducts[prodKey{p.Vendor, p.Product, p.Version, p.Unconfirmed}] = true
		}
		for _, fgProduct := range advisory.AffectedProducts {
			...
			key := prodKey{candidate.Vendor, candidate.Product, candidate.Version, candidate.Unconfirmed}
