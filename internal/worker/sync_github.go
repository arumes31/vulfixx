package worker

import (
	"context"
	"encoding/json"
	"fmt"
	"log/slog"
	"math"
	"net/http"
	"os"
	"strconv"
	"sync"
	"time"

	"github.com/jackc/pgx/v5"
	"golang.org/x/sync/errgroup"
	"golang.org/x/time/rate"
)

var (
	// githubSyncDelay is kept for compatibility with existing tests that may monkey-patch it.
	// In the refactored concurrent version, we use a rate.Limiter instead.
	githubSyncDelay = 7 * time.Second
)

func (w *Worker) syncGitHubBuzzPeriodically(ctx context.Context) {
	w.waitUntilNextRun(ctx, "github_buzz_sync", 4*time.Hour, 1*time.Minute)
	w.runWithLock(ctx, "github_buzz_sync", 2*time.Hour, w.syncGitHubBuzz)

	ticker := w.TickerFactory(4 * time.Hour)
	defer ticker.Stop()
	for {
		select {
		case <-ctx.Done():
			return
		case <-ticker.Chan():
			w.runWithLock(ctx, "github_buzz_sync", 2*time.Hour, w.syncGitHubBuzz)
		}
	}
}

type githubUpdateItem struct {
	cveID      string
	totalCount int
}

func (w *Worker) syncGitHubBuzz(ctx context.Context) {
	slog.Info("Worker: [SYNC] Starting GitHub Social Buzz synchronization...")

	rows, err := w.Pool.Query(ctx, "SELECT cve_id FROM cves WHERE published_date > NOW() - INTERVAL '14 days' ORDER BY published_date DESC")
	if err != nil {
		slog.Error("Worker: [ERROR] Failed to fetch CVEs for GitHub sync", "error", err)
		return
	}
	defer rows.Close()

	var cveIDs []string
	for rows.Next() {
		var cveID string
		if err := rows.Scan(&cveID); err != nil {
			slog.Error("Worker: Error scanning CVE row for GitHub sync", "error", err)
			continue
		}
		cveIDs = append(cveIDs, cveID)
	}
	if err := rows.Err(); err != nil {
		slog.Error("Worker: [ERROR] Row iteration error in syncGitHubBuzz", "error", err)
	}

	if len(cveIDs) == 0 {
		w.updateTaskStats(ctx, "github_buzz_sync")
		return
	}

	start := time.Now()
	var pendingUpdates []githubUpdateItem
	var mu sync.Mutex

	// Respect GitHub Search API rate limits.
	// Without authentication: 10 req/min (1 every 6s)
	// With authentication: 30 req/min (1 every 2s)
	rateInterval := 6 * time.Second
	if os.Getenv("GITHUB_TOKEN") != "" {
		rateInterval = 2 * time.Second
	}

	// If githubSyncDelay was monkey-patched to a very small value (e.g. in tests), respect that.
	if githubSyncDelay < rateInterval {
		rateInterval = githubSyncDelay
	}

	limiter := rate.NewLimiter(rate.Every(rateInterval), 1)
	g, gCtx := errgroup.WithContext(ctx)
	cveChan := make(chan string)

	// Start concurrent workers
	numWorkers := 3
	for i := 0; i < numWorkers; i++ {
		g.Go(func() error {
			for cveID := range cveChan {
				if err := limiter.Wait(gCtx); err != nil {
					return err
				}

				totalCount, err := w.fetchGitHubRepoCount(gCtx, cveID)
				if err != nil {
					slog.Error("Worker: [ERROR] GitHub request failed", "cve_id", cveID, "error", err)
					continue
				}

				mu.Lock()
				pendingUpdates = append(pendingUpdates, githubUpdateItem{cveID: cveID, totalCount: totalCount})
				mu.Unlock()
			}
			return nil
		})
	}

	// Feed workers
Loop:
	for _, cveID := range cveIDs {
		select {
		case cveChan <- cveID:
		case <-gCtx.Done():
			break Loop
		}
	}
	close(cveChan)

	if err := g.Wait(); err != nil && err != context.Canceled {
		slog.Error("Worker: [ERROR] GitHub sync workers failed", "error", err)
	}

	if len(pendingUpdates) > 0 {
		w.updateGitHubBatch(ctx, pendingUpdates)
	}

	w.updateTaskStats(ctx, "github_buzz_sync")
	slog.Info("Worker: [SYNC] GitHub Social Buzz synchronization complete.", "duration", time.Since(start), "updated_count", len(pendingUpdates))
}

func (w *Worker) fetchGitHubRepoCount(ctx context.Context, cveID string) (int, error) {
	githubURL := fmt.Sprintf("https://api.github.com/search/repositories?q=%s", cveID)

	var ghResp struct {
		TotalCount int `json:"total_count"`
	}

	resp, err := DoWithRetry(ctx, w.HTTP, RetryConfig{
		MaxRetries:  3,
		MaxWait:     5 * time.Minute,
		ShouldRetry: githubShouldRetry,
		Label:       "GitHub Buzz Sync",
	}, func() (*http.Request, error) {
		req, err := http.NewRequestWithContext(ctx, "GET", githubURL, nil)
		if err != nil {
			return nil, err
		}
		req.Header.Set("Accept", "application/vnd.github.v3+json")
		req.Header.Set("User-Agent", "Vulfixx-Threat-Intel")
		if token := os.Getenv("GITHUB_TOKEN"); token != "" {
			req.Header.Set("Authorization", "token "+token)
		}
		return req, nil
	})

	if err != nil {
		return 0, err
	}
	if resp == nil {
		return 0, fmt.Errorf("nil response from GitHub API")
	}
	defer resp.Body.Close()

	if resp.StatusCode != http.StatusOK {
		return 0, fmt.Errorf("GitHub API returned non-OK status: %d", resp.StatusCode)
	}

	if err := json.NewDecoder(resp.Body).Decode(&ghResp); err != nil {
		return 0, err
	}

	return ghResp.TotalCount, nil
}

func (w *Worker) updateGitHubBatch(ctx context.Context, updates []githubUpdateItem) {
	if len(updates) == 0 {
		return
	}

	tx, err := w.Pool.Begin(ctx)
	if err != nil {
		slog.Error("Worker: [ERROR] Failed to begin transaction for GitHub updates", "error", err)
		return
	}
	defer func() { _ = tx.Rollback(ctx) }()

	query := "UPDATE cves SET github_poc_count = $1 WHERE cve_id = $2"
	batch := &pgx.Batch{}
	for _, up := range updates {
		batch.Queue(query, up.totalCount, up.cveID)
	}

	br := tx.SendBatch(ctx, batch)
	if br != nil {
		defer br.Close()
		for i := 0; i < len(updates); i++ {
			if _, err := br.Exec(); err != nil {
				slog.Error("Worker: [ERROR] Failed to update GitHub buzz in DB via batch", "error", err)
			}
		}
	} else {
		// Fallback for pgxmock test compatibility if SendBatch is not supported
		for _, up := range updates {
			if _, err := tx.Exec(ctx, query, up.totalCount, up.cveID); err != nil {
				slog.Error("Worker: [ERROR] Failed to update GitHub buzz in DB fallback", "cve_id", up.cveID, "error", err)
			}
		}
	}

	if err := tx.Commit(ctx); err != nil {
		slog.Error("Worker: [ERROR] Failed to commit GitHub updates", "error", err)
	}
}

func githubShouldRetry(resp *http.Response, err error, attempt int) (bool, time.Duration) {
	if err != nil {
		return true, time.Duration(math.Pow(2, float64(attempt+1))) * time.Second
	}
	if resp == nil {
		return false, 0
	}

	if resp.StatusCode == http.StatusForbidden || resp.StatusCode == http.StatusTooManyRequests {
		var waitDur time.Duration
		if reset := resp.Header.Get("X-RateLimit-Reset"); reset != "" {
			if ts, err := strconv.ParseInt(reset, 10, 64); err == nil {
				waitDur = time.Until(time.Unix(ts, 0))
				if waitDur < 0 {
					waitDur = 0
				}
			}
		}
		if waitDur == 0 {
			waitDur = time.Duration(math.Pow(2, float64(attempt+1))) * time.Second
		}
		return true, waitDur
	}

	if resp.StatusCode >= 500 {
		return true, time.Duration(math.Pow(2, float64(attempt+1))) * time.Second
	}

	return false, 0
}
